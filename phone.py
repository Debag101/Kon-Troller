import socket
import threading
import time

HOST = '255.255.255.255'
PORT = 6741



#data = 'Hello World !'

#Creating a dgram (Data Gram) socket, DG = Drain Gang
client_socket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)

#Changing socket options to be a broadcast
client_socket.setsockopt(socket.SOL_SOCKET, socket.SO_BROADCAST, 1)

#timing socket out so it doesn't hang like logan paul forest incident
#client_socket.settimeout(5)

def recvfrom_host():
    while True:
        reply, address = client_socket.recvfrom(1024)
        print(f'Ping from -> {reply.decode('utf-8')}')

def find_pc():
    while True:
        data = 'controller'
        client_socket.sendto(data.encode('utf-8'), (HOST, PORT))
        time.sleep(3)


find_pc_thread = threading.Thread(target=find_pc, args=())
find_pc_thread.start()

recvfrom_host_thread = threading.Thread(target=recvfrom_host, args=())
recvfrom_host_thread.start()

